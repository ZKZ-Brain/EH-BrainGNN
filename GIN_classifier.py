...
class GINNet(nn.Module):
    def __init__(self, input_dim, output_dim, model_args,device):
        ...
        if self.num_mlp_layers > 1:
            self.mlps.append(nn.Linear(self.dense_dim2,
            ...
            self.mlps.append(nn.Linear(self.dense_dim2, output_dim))
    ...
    def forward(self, x, edge_index, batch,edge_weight = None):
        ...
        for i in range(self.num_gnn_layers):
            if edge_weight == None:
                ...
                x = self.gnn_layers[i](x, edge_index, edge_attr = edge_weight)
        ...
        graph_size = int(len(x[:,0])/self.input_dim)
        node_emb = torch.split(x, self.input_dim, dim=0)
